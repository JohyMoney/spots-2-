...
const deleteConfirmBtn = deleteModal.querySelector(".modal_button[type='submit']");
const deleteCancelBtn = deleteModal.querySelector(".modal_button[type='button']");
...
function isCardLikedByCurrentUser(cardData) {
  if (!cardData.likes || !currentUserId) {
    return false;
  }

  return cardData.likes.some((user) => user && user._id === currentUserId);
}
...
function getCardElement(data) {
  ...
  if (isCardLikedByCurrentUser(data)) {
  ...
  cardLikeBtn.addEventListener("click", () => {
    const request = isCardLikedByCurrentUser(data)
    ...
    request
      .then((updatedCard) => {
        ...
        cardLikeBtn.classList.toggle("card__like-button_active", isCardLikedByCurrentUser(data));
      })
      .catch((err) => {
        console.log(err);
      });
